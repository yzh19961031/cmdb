<template>
  <div class="app-container" style="display: flex">
    <el-dialog
      :title="isGroupEditing ? '编辑分组' : '新增分组'"
      width="600px"
      :visible.sync="dialogVisible"
      :before-close="(done) => handleClose(done, 'groupForm')"
    >
    ...
    <el-dialog
      :title="isModelEditing ? '编辑模型' : '新增模型'"
      width="600px"
      :visible.sync="modelDialogVisible"
      :before-close="(done) => handleClose(done, 'modelForm')"
    >
    ...
    <el-dialog
      ...
      :before-close="(done) => handleClose(done, 'relationForm')"
    >
      <el-form :model="relationForm" ref="relationForm" :rules="relationRules">
        ...
        >
          <el-select
            v-model="relationForm.relationTypeId"
            placeholder="请选择活动区域"
          >
            <el-option
        ...
        <el-form-item label="关系约束" label-width="120px" prop="relationBind">
          <el-select
            v-model="relationForm.relationBind"
            placeholder="请选择关系约束"
          >
            <el-option
              v-for="item in relationBindList"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitForm('relationForm')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <!-- 唯一校验 -->
    <validation
      :validationDialogVisible="validationDialogVisible"
      :attrList="attrTableData"
      :modelId="+currentModelId"
    />

    <!-- 模型属性 -->
    <attribute
      :attrDialogVisible="attrDialogVisible"
      :isAttrEditing="isAttrEditing"
      :modelId="+currentModelId"
      :attrForm="attrForm"
    />
    ...
    <div class="sidebar" style="width: 240px; border-right: 1px solid #ebeef5">
      ...
      >
        ...
        >
            ...
            <div class="dialog-title">
              <div>
                ...
                <span style="font-weight: bold"
                  >{{ menu.name }}({{ menu.resourceModelList.length }})</span
                >
              </div>
              <div class="icon-container">
                ...
                <el-tooltip content="删除分组" placement="top">
                  <i
                    ...
                    @click="deleteGroup(menu.id, $event)"
                  ></i>
                </el-tooltip>
              </div>
            </div>
          ...
          <el-menu-item-group
            v-if="
              menu.resourceModelList.some((model) => model.isEnabled === true)
            "
          >
            <template slot="title">已启用</template>
            <el-menu-item
              v-for="submenu in menu.resourceModelList.filter(
                (model) => model.isEnabled === true
              )"
              :key="submenu.id + ''"
              :index="submenu.id + ''"
            >
              <div
                class="submenu-item dialog-title"
                @mouseenter="showIcons(submenu.id)"
                @mouseleave="hideIcons(submenu.id)"
              >
                {{ submenu.name }}
                <div v-if="isHovering(submenu.id)">
                  ...
                  >
                    <i
                      class="el-icon-edit"
                      style="color: blue; font-size: 15px"
                      @click="updateModel(menu.id, submenu, $event)"
                    ></i>
                  </el-tooltip>
                  <el-tooltip content="删除模型" placement="top">
                    <i
                      class="el-icon-delete"
                      style="color: red; font-size: 15px"
                      @click="deleteModel(submenu.id, $event)"
                    ></i>
                  </el-tooltip>
                </div>
              </div>
          ...
          <el-menu-item-group
            v-if="
              menu.resourceModelList.some((model) => model.isEnabled === false)
            "
          >
      ...
      <el-main style="padding-left: 10px; padding-top: 12px">
        ...
        >
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="small" type="text" @click="detailAttr(scope.row)"
                >详情</el-button
              >
              <el-tooltip
                v-if="scope.row.identifier === 'instance_name'"
                content="固定属性，无法编辑"
                placement="top"
              >
              ...
              <template v-else>
                <el-button
                  size="small"
                  type="text"
                  @click="updateAttr(scope.$index, scope.row)"
                  >编辑</el-button
                >
                <el-button
                  style="color: red"
        ...
        >
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-tooltip
                v-if="String(scope.row.sourceId) !== currentModelId"
                content="被动关联，无法删除"
                placement="top"
              >
                <!-- 如果是目标端模型的话 不允许删除 -->
              ...
              <el-button
                ...
                size="small"
                type="text"
                @click="deleteRelation(scope.$index, scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      modelRules: {
        name: [
          { required: true, message: "请输入模型名称", trigger: "blur" },
          {
            min: 2,
            max: 50,
            message: "模型名称长度必须在2到50个字符之间",
            trigger: "blur",
          },
        ],

        uniqueKey: [
          { required: true, message: "请输入模型唯一标识", trigger: "blur" },
          {
            min: 2,
            max: 50,
            message: "模型唯一标识长度必须在2到50个字符之间",
            trigger: "blur",
          },
          {
            pattern: /^[a-zA-Z][a-zA-Z0-9_]*$/,
            message: "模型唯一标识只允许字母开头，包含字母，数字和下划线",
            trigger: "change",
          },
        ],

        description: [
          { max: 500, message: "模型描述长度不能超过500位", trigger: "blur" },
        ],
      ...
      relationRules: {
        ...
        relationBind: [
          { required: true, message: "请选择关系约束", trigger: "blur" },
        ],
      },
      allModelList: [],
      relationBindList: [
  ...
  computed: {
    currentModel() {
      return this.allModelList.find(
        (model) => String(model.id) === this.currentModelId
      );
    },
  },
  methods: {
    ...
    async addModelRelation() {
      const [listAllResponse, listResponse] = await Promise.all([
        listAll(),
        list(),
      ]);
